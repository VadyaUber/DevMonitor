...
SerialPort::SerialPort(string devUSB, uint32_t baud_rate)
{
    openserialport(devUSB, baud_rate);
}

int SerialPort::openserialport(string devUSB, uint32_t baud_rate)
{
    //открытие порта
...
int SerialPort::writeserial(char* buff, size_t size)
{
    write(serial_port, buff, size);
}

int SerialPort::readserial(uint8_t buf_read, size_t size)
{
...
uint16_t SerialPort::CRC16(uint8_t* data, uint16_t size)
{
